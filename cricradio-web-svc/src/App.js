...
class App extends Component{
    ...
    createConsumer() {
        fetch(`http://kafka-rest:38082/consumers/${this.state.group}`,{
            method:"POST",
    ...
    componentDidMount() {
        fetch("http://cricradio-go-svc/matches/list")
            .then(res => res.json())
            .then(
                (result) => {
                    this.setState({
    ...
    async ListenComm(matchId){
        ...
        await fetch(`http://kafka-rest:38082/consumers/${this.state.group}/instances/${this.state.instance}/subscription`,{
            method: "POST",
            headers: { 'Content-Type': 'application/vnd.kafka.json.v2+json' },
            body:JSON.stringify({
                "topics":[matchId]
    ...
    consume(){
        ...
        fetch(`http://kafka-rest:38082/consumers/${this.state.group}/instances/${this.state.instance}/records`,{
            method:"GET",
